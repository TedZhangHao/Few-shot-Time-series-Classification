...
class AugmentedConv(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, dk, dv, Nh, shape=0, relative=False, stride=1):
        ...
        # self.bn = nn.BatchNorm2d(self.dv)
        ...
        # self.conv_out = nn.Conv2d(1, 16, self.kernel_size, stride=stride, padding=self.padding)
        #
        # self.qkv_conv = nn.Conv2d(1, 2 * self.dk + self.dv, kernel_size=self.kernel_size, stride=stride, padding=self.padding)
        #
        # self.attn_out = nn.Conv2d(16, 16, kernel_size=1, stride=1)
        if self.relative:
    ...
    def forward(self, x):
        ...
        # return self.bn(self.torch.add(conv_out,attn_out))
